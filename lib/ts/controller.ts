...
export const Controller = (message: Message, prefix: string, bot: Client): void | Promise<void | Message> => {
    ...
    if (message.content.substr(0, prefix.length) === prefix && message.content.substr(0, 3) !== '!!!') {
        ...
        if (message.content.length > prefix.length) {
            ...
            if (command === 'go') {
                return Player.go(message, words);
            }
            if (command === 'set' || command === 'settings') {
                return Settings.instantiate(message, words);
            }
            if (command === 'd' || command === 'dd' || command === 'drago' || command === 'dragodinde' || command === 'dragodindes') {
                return Dragodinde.controller(message, words);
            }
            if (command === 'current' || command === 'now') {
                return Player.current(message);
            }
            if (command === 'join') {
                return Player.joinChannel(message);
            }
            if (command === 'repeat') {
                return Player.toggleLoop(message);
            }
            if (command === 'cancel') {
                return Player.cancel(message);
            }
            if (command === 'quit') {
                return Player.stop(message);
            }
            if (command === 'stop') {
                return Player.stop(message, false);
            }
            if (command === 'pause') {
            ...
            if (command === 'radio') {
                return Radio.instantiate(message, words);
            }
            if (command === 'remove') {
                return MessageFunc.instantiate(message, words);
            }
            if (command === 'clear') {
                return MessageFunc.instantiate(message, words, 'all');
            }
            if (command === 'help') {
                return Helper.instantiate(message, words);
            }
            if (command === 'pioupiou') {
                return Custom.pioupiou(message);
            }
            if (command === 'admin') {
                return Admin.instantiate(message, words, bot);
            }
