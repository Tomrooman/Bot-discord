...
export default class Streams {
    ...
    setStream(T, game, allSettings, bot) {
        let followId = game === 'wolcen' ? Config.twitter.wolcen.id : Config.twitter.warzone.id;
        streams.push(T.stream('statuses/filter', { follow: followId, tweet_mode: 'extended' }));
        streams[streams.length - 1].on('tweet', tweet => {
            if (tweet.user.id_str === Config.twitter.wolcen.id || tweet.user.id_str === Config.twitter.warzone.id) {
                this.sendReceivedTweet(game, allSettings, bot);
            }
        });
    }

    sendReceivedTweet(game, bot) {
